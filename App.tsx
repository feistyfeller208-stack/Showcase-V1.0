    ...
    <div className="flex flex-col min-h-screen md:pl-72 bg-slate-50/50">
      ...
      <aside className="hidden md:flex flex-col w-72 h-full bg-white border-r fixed left-0 top-0 p-10 z-30">
        <div className="flex items-center gap-4 mb-12">
          <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center text-white font-black text-2xl shadow-xl shadow-blue-100">S</div>
        ...
        <nav className="space-y-2 flex-1">
          ...
          <NavLink active={isActive('/test')} icon={<PlusIcon className="w-5 h-5" />} label="Hello Test" onClick={() => navigate('/test')} />
        </nav>
      ...
      {/* Mobile Nav */}
      <nav className="md:hidden fixed bottom-6 left-6 right-6 bg-slate-900 text-white rounded-3xl flex justify-around p-4 z-50 shadow-2xl">
        <button onClick={() => navigate('/')} className="p-2 text-slate-400"><HomeIcon className="w-6 h-6" /></button>
        <button onClick={() => navigate('/create')} className="p-2 text-slate-400"><PlusIcon className="w-6 h-6" /></button>
        <button onClick={() => navigate('/analytics')} className="p-2 text-slate-400"><ChartBarIcon className="w-6 h-6" /></button>
      </nav>

      <main className="flex-1 p-6 md:p-14 max-w-7xl mx-auto w-full pb-32 md:pb-14 animate-up">
        {children}
      </main>
    </div>
  );
...
const App: React.FC = () => {
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center bg-white"><div className="w-10 h-10 border-4 border-blue-600 border-t-transparent rounded-full animate-spin"></div></div>;
  }
  ...
  return (
    <HashRouter>
      <Routes>
        ...
        <Route path="/test" element={user ? <AppLayout logout={signOut}><Hello /></AppLayout> : <Navigate to="/login" />} />
